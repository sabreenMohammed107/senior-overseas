			  			...
			  			<table id="courseEval" class="dattable table table-striped thead-dark  w-100">
                            ...
                            <tr>
								...
								<td>{{$row->currency_name}}</td>
			  						<td>
                                      <a href="{{ route('currency.edit',$row->id) }}" class="btn btn-info d-inline-block">edit</a>
                                    <a href="#" onclick="destroy('this currency','{{$row->id}}')" class="btn d-inline-block btn-danger">delete</a>
                                    <form id="delete_{{$row->id}}" action="{{ route('currency.destroy', $row->id) }}" method="POST" style="display: none;">
                                        @csrf
                                        @method('DELETE')
